<template>
  ...
  <div class="buttons">
    <el-radio-group v-model="displayType" size="small">
      <el-radio-button v-for="(item,index) in displayTypes" :key="index" :label="index">{{item.alias}}</el-radio-button>
    </el-radio-group>
    <input type="text" v-model="keyword">
  </div>
  <ul class="list">
    <li class="country" v-for="country in sortedCountries" :key="country.CountyrCode">
...
<style scoped>
.buttons{
  margin-top:1em;display:flex;justify-content:space-between;
}
.list{
  list-style:none;
  display:grid;
...
.country img{max-width:1em;margin-right:1em;}
.country .namme{margin-right:1em;font-weight:bold;}
</style>
